...
export default function UpdateListing() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (form.offer && form.discountPrice >= form.regularPrice) {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) toast.error(data.message || 'Failed to Up listing');
      else {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-6">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-4xl space-y-6">
        <h1 className="text-3xl font-bold text-center text-blue-600">Update Listing</h1>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            {['sell', 'rent'].map((type) => (
              <label key={type} className="flex items-center gap-2">
                <input type="radio" name="type" value={type} onChange={handleChange} checked={form.type === type} />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              Bathroom
              <input type="number" name="bathroom" min={1} max={6} onChange={handleChange} value={form.bathroom} className="px-4 py-2 border rounded-md w-full" />
            </div>
            <div>
              Bedroom
              <input type="number" name="bedroom" min={1} max={6} onChange={handleChange} value={form.bedroom} className="px-4 py-2 border rounded-md w-full" />
            </div>
          </div>
